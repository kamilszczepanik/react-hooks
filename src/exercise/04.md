...
This code is working with arrays in JavaScript, specifically using the methods
slice() and the spread operator ....

const newHistory = history.slice(0, currentStep + 1)

Here, the slice() method is used on the history array. slice() creates a new
array from a portion of an existing array. It takes two parameters: the start
index and the end index (which is exclusive - meaning it goes up to, but does
not include, the end index). In this case, slice(0, currentStep + 1) is creating
a new array from the start of history up to the index currentStep + 1. This new
array is then assigned to the variable newHistory. This is useful when you want
to create a sub-array from an existing array.

const squares = [...currentSquares]

The spread operator ... is used here to create a new array that is a copy of
currentSquares. This is a simple way to clone an array in JavaScript. This
operation is particularly helpful when you want to work with the values of the
existing array but don't want to modify the original array. By creating a clone,
you can manipulate squares without affecting currentSquares.

So, in simpler terms:

The first line says, "Let's remember only a part of our history up to a certain
point and forget everything that happened after that." The second line says,
"Let's make a copy of the current state of squares so we can change it without
