...
### flexible Local storage custom hook - could be really helpfull

### custom hooks to reuse logic in different places of application, or to clean up the code better

### Callback function

A callback function is like setting a reminder for yourself. Let's say you need
to go buy groceries, but before that, you want to finish reading a book. You
might say to yourself, "As soon as I finish reading this book, I'm going to go
buy groceries." In this case, "go buy groceries" is your reminder or "callback"
that you want to do after reading the book.

In the programming world, it works in a similar way. A callback function is a
function that you pass as an argument to another function, with the intention of
having it called (or "executed") at some point in the future.

For example, imagine you have a function called doSomething, and you want to do
something else (callMeBack) after doSomething has finished executing:

```
function doSomething(callMeBack) {
  // Here we do some complicated task that takes some time...

  // And once we're done, we call our callback function
  callMeBack();
}
...
// You pass `iAmCallback` as an argument to `doSomething`
doSomething(iAmCallback);
```

In this example, iAmCallback is the callback function. When we call
doSomething(iAmCallback);, we're saying "Run doSomething and once you're done,
run iAmCallback".

Callback functions are a fundamental concept in JavaScript, and they are widely
used for handling asynchronous operations, which are operations that we know
will take some time to complete, like downloading an image, fetching data from a
server, reading a file, etc. Elaborate on your learnings here in
`src/exercise/02.md`

### You can pass function to react useState and it will only be called when needed
